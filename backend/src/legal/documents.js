...
function privacySections(config, options = {}) {
  ...
  return [
    ...
    [
      ...
      '<p>Railway aloja el backend y PostgreSQL. El proveedor de IA configurado, OpenAI o Google Gemini, recibe las instrucciones, el catálogo y el historial reciente para generar respuestas y aplicar sus controles de seguridad. No le enviamos la contraseña ni las credenciales de acceso del dueño. Durante pruebas puede utilizarse el nivel gratuito de Gemini, sujeto a las condiciones de Google sobre el uso de contenido para mejorar sus productos; por ello debe limitarse a cuentas y mensajes de prueba. Un mensaje o comentario puede contener información personal escrita por un cliente: evita introducir información sensible que no sea necesaria para atenderlo.</p><p>Si eliges iniciar sesión con Google o Facebook, el proveedor comparte el identificador de esa cuenta, tu nombre y correo verificado para crear o vincular tu cuenta de Wassa517. Wassa517 no recibe la contraseña del proveedor y no conserva su token de acceso después de completar el inicio de sesión.</p><p>Meta procesa los identificadores de cuentas y páginas, teléfonos, mensajes de WhatsApp y Messenger, mensajes directos y comentarios de Instagram, así como sus estados, para entregar y responder las comunicaciones autorizadas. Los tokens de páginas conectadas se almacenan cifrados y se eliminan al desconectar la página o borrar la cuenta. Al mostrar una imagen de producto, la app solicita el archivo al host de su URL HTTPS, que recibe la petición de red. GitHub Pages aloja estas páginas informativas cuando se publican allí; el formulario envía las credenciales al backend de Wassa517, no a un repositorio de GitHub.</p><p>Los proveedores pueden tratar información fuera de tu país, conforme a sus contratos y políticas. Consulta <a href="https://policies.google.com/privacy" rel="noopener noreferrer">Google</a>, <a href="https://ai.google.dev/gemini-api/terms" rel="noopener noreferrer">condiciones de Gemini API</a>, <a href="https://openai.com/policies/privacy-policy/" rel="noopener noreferrer">OpenAI</a>, <a href="https://developers.openai.com/api/docs/guides/your-data" rel="noopener noreferrer">controles de datos de su API</a>, <a href="https://www.facebook.com/privacy/policy/" rel="noopener noreferrer">Meta</a>, <a href="https://www.whatsapp.com/legal/privacy-policy" rel="noopener noreferrer">WhatsApp</a>, <a href="https://railway.com/legal/privacy" rel="noopener noreferrer">Railway</a> y <a href="https://docs.github.com/en/site-policy/privacy-policies/github-general-privacy-statement" rel="noopener noreferrer">GitHub</a>.</p>',
    ],
    [
      'Almacenamiento y seguridad',
      '<p>La app conserva el token de sesión y el perfil en las preferencias locales para mantener el acceso. Al cerrar sesión o eliminar la cuenta, elimina esa sesión local. Las conexiones públicas de Wassa517 utilizan HTTPS; las contraseñas se guardan con bcrypt y las consultas de cada negocio requieren autenticación y se limitan a sus recursos.</p><p>El backend necesita leer los mensajes para responder con IA. El tratamiento dentro de Wassa517 no es cifrado de extremo a extremo entre el cliente y el proveedor de IA. La app no solicita acceso a contactos, SMS, ubicación, cámara o micrófono del teléfono.</p>',
    ],
    [
      'Conservación',
    ...
    [
      ...
      `<p>Puedes eliminar la cuenta en <strong>Configuración → Eliminar mi cuenta</strong> o en <a href="${path.deletion}">el formulario de eliminación</a>, sin reinstalar la app. Se verifica tu contraseña o, si entraste con Google o Facebook, tu sesión social autenticada. Después se borran de PostgreSQL activo la cuenta, catálogo, ajustes, conexión, conversaciones, mensajes, reportes de IA y trabajos relacionados. La eliminación es inmediata cuando el servidor la confirma y no se puede deshacer.</p><p>El formulario web usa la contraseña de Wassa517; las cuentas creadas solo con acceso social pueden eliminarse desde la app o solicitar asistencia. No cancela suscripciones externas ni elimina mensajes ya enviados en WhatsApp. Para solicitar acceso, corrección, eliminación de datos concretos o información sobre copias, escribe a ${contact(config)}. No envíes contraseñas ni credenciales de OpenAI o Meta por correo.</p>`,
    ],
    [
